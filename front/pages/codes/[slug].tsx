...
// hook
import useCodeSwr from "../../hooks/swr/useCodeSwr";
// component
...
const Post: NextPage<{ slug: string; staticPost: PostType }> = ({
  ...
  const post = useCodeSwr({ id: slug, staticPost });
  return (
    <>
      <Head>{fullHead && fullHead}</Head>
      <Layout>
        <div className="w-main mx-auto">
          <article>
            ...
            <div className="flex mb-4">
              <div className="mr-3">
                <Link href={`/codes/category/${post!.category.slug}`}>
...
export async function getStaticPaths() {
  const paths = await CodeService.getAllSlugList();
  return {
    paths,
    fallback: true,
  };
...
export async function getStaticProps({
  ...
  const staticPost = await CodeService.getOne({ id: slug });
  if (!staticPost) {
