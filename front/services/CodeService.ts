...
class CodeService {
  static async getSeo({ slug }: { slug: string }) {
    try {
      const res = await RepositoryFactory.code.getSeo({ slug });
  ...
  static async getSeoByCategorySlug({
    ...
    try {
      const res = await RepositoryFactory.code.getSeoByCategorySlug({
        slug,
      });
      const data = res.data.data.codeCategories.edges[0].node.seo;
      const seoInfo: SeoType = {
        metaFullHead: this._replaceHostNameForSeo(data.fullHead),
      };
      return seoInfo;
